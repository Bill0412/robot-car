...
class Car:
    ...
    def move(self, destination, dest_direction):
            ...
            if x != 0 and not map[ self.direction[0]+x ][ self.direction[1] ]:
                possible_directions.append((x, 0))

            if y != 0 and not map[ self.direction[0] ][ self.direction[1]+y ]
                possible_directions.append((0, y))

            # if there's obstacle, change direction
            if self.direction in possible_directions:
                 self.__follow_line()
                 move(destination,dest_direction)

            elif len(possible_directions):   # or (len(possible_directions) == 2 and self.direction not in possible_directions):
                #[element for element in elements if element == target]
                while self.direction != possible_directions[0]
                    self.direction = direction_space[ (direction_space.index(self.direction)+1)%4 ]
                    turn90(0.6)
                move(destination,dest_direction)
                ...
                if not x and y :
                    move( ( self.direction[0] , self.direction[1]+1 ) , dest_direction)
                    move( ( self.direction[0]+2 , self.direction[1]+1 ) , dest_direction)
                    move(destination,dest_direction)
                if not y and x :
                    move( ( self.direction[0]+1, self.direction[1] ) , dest_direction)
                    move( ( self.direction[0]+1 , self.direction[1]+2 ) , dest_direction)
                    move(destination,dest_direction)
